...
public:
Parser parser;
int rank, nProcs, i, j, dump_index;
int nWorkers, instance, local_rank, min_valid;
...
GathererTemplate g;
SimulatorTemplate sim;

void init(MPI_Comm &world,std::string parser_file) {
  ...
  // ******************* SET UP WORKERS ***************************************
  nWorkers = nProcs / parser.CoresPerWorker;
  instance = rank / parser.CoresPerWorker;
  local_rank = rank % parser.CoresPerWorker;
  min_valid = int(parser.redo_thresh*nWorkers*parser.nRepeats);
  ...
  g.init(parser,nWorkers,dump_index,rank);
  MPI_Bcast(&(g.initialized),1,MPI_INT,0,world);
  if(g.initialized==0) exit(-1);

  sim.init(instance_comm,parser,g.params,instance);
